...
register. Its second argument (decay) is an *exact* match for mmlgb's
decreasing `@ve` in every case: it writes the hardware envelope directly,
starting at the note's target volume, decreasing at the given pace, and
lets it run to completion in hardware with no further software
involvement -- structurally identical to what mmlgb does.

Its first argument (attack) has two distinct modes now (code/audio.s's
`env0-e7` vs `e8-ef`, i.e. bit 3 of the attack nibble):

- When the current volume is 0, `e8-ef` is a genuine hardware fade-in --
  starts at volume 0, climbs to 15 entirely in hardware at the given pace.
  This is an *exact* match for mmlgb's increasing `@ve`, which also always
  sweeps from the current `v` (here, 0) to 15.
- At any other current volume, no engine feature reproduces mmlgb's
  (v -> 15) sweep -- the only other attack option (`e0-e7`, the original,
  software-orchestrated one, traced through `handleEnvelopes`/
  `getWaitTimeForEnvelope`) hard-codes its *starting* volume to 1 (not
  whatever `vol` last set) and only ramps *up to* the target volume, using
  its own `envelopeWaitTable` (indexed by target volume, capped at 13 --
  volumes 14 and 15 have no table row at all, a real limitation in the
  reference engine itself) to decide how long that takes before forcing
  the exact target. The closest available approximation here is to pick
  whichever attack pace makes this engine's own (1 -> target) sweep take
  about the same real-world *duration* as mmlgb's (v -> 15) sweep would
  have, so the swell's speed reads similarly even though its range and
  starting point don't match. This is what the rest of this module (still)
  computes; see emitter.py's `_emit_env` for which mode gets picked.
"""
