...
_TICK_KINDS = {'note', 'noise_literal', 'rest', 'wait', 'release', 'raw_freq'}
# Field index (Ev.a/.b/.c/.d) holding the ticks value, per kind.
_TICKS_FIELD = {'note': 'c', 'noise_literal': 'b', 'rest': 'a', 'wait': 'a', 'release': 'a',
                'raw_freq': 'b'}
...
def resolve_tempo(trees: List[list]) -> List[List[float]]:
    ...
    # None, or ('ticks'|'seconds', remaining, accumulated_exact_frames) for
    # a tick-consuming event that's been partially resolved already because
    # some other channel's tempo change landed inside it. A frame-exact
    # event (mmlgb's `=X` syntax -- see Ev.frame_exact) has a real-world
    # duration that's completely independent of tempo, so it's tracked as
    # remaining *seconds* rather than remaining *ticks*; a tempo change
    # landing inside one doesn't change how long it takes at all, it just
    # needs its already-elapsed portion converted to seconds like anything
    # else for cross-channel chronology.
    in_progress: List[Optional[Tuple[str, float, float]]] = [None, None, None, None]
    resolved: List[List[float]] = [[], [], [], []]
    global_bpm = DEFAULT_TEMPO_BPM

    def fresh_state(ev) -> Tuple[str, float, float]:
        if ev.frame_exact:
            frames = getattr(ev, _TICKS_FIELD[ev.kind])
            return ('seconds', frames / GB_FRAME_RATE, 0.0)
        return ('ticks', getattr(ev, _TICKS_FIELD[ev.kind]), 0.0)

    def projected_completion(mode: str, remaining: float) -> float:
        if mode == 'seconds':
            return remaining
        return remaining * real_seconds_per_tick(global_bpm)
    ...
    def checkpoint(other: int, at_time: float):
        ...
        if in_progress[other] is not None:
            mode, remaining, accum = in_progress[other]
        else:
            oev = flat[other][cursors[other]]
            if oev.kind not in _TICK_KINDS:
                return
            mode, remaining, accum = fresh_state(oev)
        elapsed_time = at_time - real_time[other]
        if mode == 'seconds':
            coverable = min(remaining, elapsed_time)
            frames_partial = coverable * GB_FRAME_RATE
        else:
            spt = real_seconds_per_tick(global_bpm)
            coverable = min(remaining, elapsed_time / spt)
            frames_partial = coverable * spt * GB_FRAME_RATE
        in_progress[other] = (mode, remaining - coverable, accum + frames_partial)
    ...
    while True:
        ...
        for ch in active:
            if in_progress[ch] is not None:
                mode, remaining, _ = in_progress[ch]
                t = real_time[ch] + projected_completion(mode, remaining)
                boundaries.append((t, ch, False))
                continue
            ev = flat[ch][cursors[ch]]
            if ev.kind in _TICK_KINDS:
                mode, remaining, _ = fresh_state(ev)
                t = real_time[ch] + projected_completion(mode, remaining)
                boundaries.append((t, ch, False))
            else:
        ...
        if in_progress[ch] is not None:
            mode, remaining, accum = in_progress[ch]
        else:
            mode, remaining, accum = fresh_state(ev)
        if mode == 'seconds':
            exact_frames = accum + remaining * GB_FRAME_RATE
            real_time[ch] += remaining
        else:
            spt = real_seconds_per_tick(global_bpm)
            exact_frames = accum + remaining * spt * GB_FRAME_RATE
            real_time[ch] += remaining * spt
